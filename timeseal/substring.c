 ...
 * Based on kallasyms.c from the Linux kernel.
 *
 * Copyright 2002       by Kai Germaschewski
 * Copyright 2010 Wil Mahan
 *
...
usage(void)
{
	fprintf(stderr, "Usage: substring < fics.txt > dict.txt\n");
	exit(1);
}
...
write_src(void)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		printf("name 0x%02x ", table[i].len);
		for (k = 0; k < table[i].len; k++)
			printf("%c", table[i].sym[k]);
		printf("\n");
...
static void learn_token(unsigned char *data, int len)
{
	...
	for (i = len; i >= 2; i--) {
		...
		if (!ptr) {
			...
			/* when we create an entry, its profit is 0 because
